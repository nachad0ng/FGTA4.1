...
const tbl_list = $('#pnl_editKomponengrid-tbl_list')
const txt_title = $('#pnl_editKomponengrid-title')
const pnl_control = $('#pnl_editKomponengrid-control')
const btn_removechecked  = $('#pnl_editKomponengrid-removechecked')
const btn_addrow = $('#pnl_editKomponengrid-addrow')
...
export function createnew(data, options) {
	...
	grd_list.clear()
	txt_title.html(data.employee_name)
	header_data = data
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/Komponen-list`
		options.criteria['id'] = data.employee_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editKomponenform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editKomponenform', () => {
		$ui.getPages().ITEMS['pnl_editKomponenform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/Komponen-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editKomponenform', ()=>{
		$ui.getPages().ITEMS['pnl_editKomponenform'].handler.createnew(header_data)
	})	
}