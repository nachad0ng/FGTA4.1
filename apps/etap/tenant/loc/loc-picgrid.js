...
const tbl_list = $('#pnl_editpicgrid-tbl_list')
const txt_title = $('#pnl_editpicgrid-title')
const pnl_control = $('#pnl_editpicgrid-control')
const btn_removechecked  = $('#pnl_editpicgrid-removechecked')
const btn_addrow = $('#pnl_editpicgrid-addrow')
...
export function OpenDetil(data) {
	...
	grd_list.clear()
	txt_title.html(data.loc_name)
	header_data = data

	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pic-list`
		options.criteria['id'] = data.loc_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpicform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpicform', () => {
		$ui.getPages().ITEMS['pnl_editpicform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pic-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpicform', ()=>{
		$ui.getPages().ITEMS['pnl_editpicform'].handler.createnew(header_data)
	})	
}