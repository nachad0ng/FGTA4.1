...
const tbl_list = $('#pnl_editbrandgrid-tbl_list')
const txt_title = $('#pnl_editbrandgrid-title')
const pnl_control = $('#pnl_editbrandgrid-control')
const btn_removechecked  = $('#pnl_editbrandgrid-removechecked')
const btn_addrow = $('#pnl_editbrandgrid-addrow')
...
export function createnew(data, options) {
	...
	grd_list.clear()
	txt_title.html(data.company_id)
	header_data = data
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/brand-list`
		options.criteria['id'] = data.company_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbrandform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbrandform', () => {
		$ui.getPages().ITEMS['pnl_editbrandform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/brand-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbrandform', ()=>{
		$ui.getPages().ITEMS['pnl_editbrandform'].handler.createnew(header_data)
	})	
}