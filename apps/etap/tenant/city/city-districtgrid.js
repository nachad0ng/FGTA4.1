...
const tbl_list = $('#pnl_editdistrictgrid-tbl_list')
const txt_title = $('#pnl_editdistrictgrid-title')
const pnl_control = $('#pnl_editdistrictgrid-control')
const btn_removechecked  = $('#pnl_editdistrictgrid-removechecked')
const btn_addrow = $('#pnl_editdistrictgrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/district-list`
		options.criteria['id'] = data.city_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdistrictform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdistrictform', () => {
		$ui.getPages().ITEMS['pnl_editdistrictform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/district-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdistrictform', ()=>{
		$ui.getPages().ITEMS['pnl_editdistrictform'].handler.createnew(header_data)
	})	
}