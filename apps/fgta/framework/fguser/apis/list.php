...
class DataList extends WebAPI {
	...
	public function execute($options) {
		...
		try {
			...
			$stmt = $this->db->prepare("select count(*) as n from fgt_user A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				user_id, user_name, user_fullname, user_email, user_password, user_disabled, group_id, _createby, _createdate, _modifyby, _modifydate 
				from fgt_user A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
