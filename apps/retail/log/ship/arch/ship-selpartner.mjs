...
const btn_load = $('#pnl_selpartner-btn_load')
const tbl_list = $('#pnl_selpartner-tbl_list')
const txt_search = $('#pnl_selpartner-txt_search')
...
function btn_load_click() {
	...
	var fn_listloading = async (options) => {
		options.api = $ui.apis.loadpartner
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.mask('setdata')
	$ui.getPages().show('pnl_edit')
	$ui.getPages().ITEMS['pnl_edit'].handler.txt_partner_id_select(record.partner_id, record.partner_name)
