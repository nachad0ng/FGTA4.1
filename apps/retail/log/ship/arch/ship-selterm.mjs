...
const btn_load = $('#pnl_selterm-btn_load')
const tbl_list = $('#pnl_selterm-tbl_list')
const txt_search = $('#pnl_selterm-txt_search')
...
function btn_load_click() {
	...
	var fn_listloading = async (options) => {
		options.api = $ui.apis.loadshipterm
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.mask('setdata')
	$ui.getPages().show('pnl_edit')
	$ui.getPages().ITEMS['pnl_edit'].handler.txt_logshipterm_id_select(record.logshipterm_id, record.logshipterm_name)
