...
const btn_load = $('#pnl_selmethod-btn_load')
const tbl_list = $('#pnl_selmethod-tbl_list')
const txt_search = $('#pnl_selmethod-txt_search')
...
function btn_load_click() {
	...
	var fn_listloading = async (options) => {
		options.api = $ui.apis.loadshipmethod
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.mask('setdata')
	$ui.getPages().show('pnl_edit')
	$ui.getPages().ITEMS['pnl_edit'].handler.txt_logshipmethod_id_select(record.logshipmethod_id, record.logshipmethod_name)
