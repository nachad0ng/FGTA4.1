	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"sea_id" => " sea_id = :sea_id "
				]
			);

			$sql = \FGTA4\utils\SqlUtility::Select('mst_sea', [
			...
			$result->record = array_merge($record, [
				'sea_datestart' => date("d/m/Y", strtotime($record['sea_datestart'])),
				'sea_dateend' => date("d/m/Y", strtotime($record['sea_dateend'])),
