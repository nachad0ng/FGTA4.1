...
const tbl_list = $('#pnl_editdetailgrid-tbl_list')
const txt_title = $('#pnl_editdetailgrid-title')
const pnl_control = $('#pnl_editdetailgrid-control')
const btn_removechecked  = $('#pnl_editdetailgrid-removechecked')
const btn_addrow = $('#pnl_editdetailgrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/detail-list`
		options.criteria['id'] = data.journal_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdetailform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdetailform', () => {
		$ui.getPages().ITEMS['pnl_editdetailform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/detail-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdetailform', ()=>{
		$ui.getPages().ITEMS['pnl_editdetailform'].handler.createnew(header_data)
	})	
}